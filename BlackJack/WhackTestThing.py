...
while chips > 0:
    ...
    frame1=ttk.Frame(root).place(x=100,y=0)
    frame2=ttk.Frame(root).place(x=200, y=0)
    frame3=ttk.Frame(root).place(x=300, y=0)
    frame4=ttk.Frame(root).place(x=400, y=0)
    ...
    while index<2:
        ...
        index = index + 1
    for thing in display_cards:#could be very wrong
        label = Label(frames[interger],image=thing)
        label.pack()
        interger=interger+1
