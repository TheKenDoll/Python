...
while chips > 0:
    ...
    bet = int(input('What\'s your wager? '))
    

    if bet > chips:
        print ('You don\'t have enough chips. Please place a value %s or less.' % chips)
        bet = int(input('What\'s your wager?'))
    ...
    while True:
        ...
        def print_list(x):#dont really know how this works but it works just to make it print nicer jk now i know the \n means put on next line
            print('\n'.join(x))
    ...
    while stand == True:
        ...
        sum_of_dealers_cards = (sum(cards_chosen_values[len(cards_chosen_values)-2-dealer_hit:len(cards_chosen_values)]))#need to change it to take the value starting from 5#takes the valuse from the end of the list
        print ('For a total of: %s' % sum_of_dealers_cards)#
    ...
    if win_lose == 'win':
        ...
        print ('You lost.')
    print ('You currently have %s' % chips)
...
if chips <= 0:
    print ('Your out of chips')
    print ('GAME OVER')
else:
    print ('Taking some money home I see.')
